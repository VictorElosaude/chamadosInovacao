    ...
    if (imageContent) {
        ...
        document.addEventListener('mousemove', function (e) {
            if (isResizing) {
                ...
                const maxWidth = window.innerWidth - imageContent.getBoundingClientRect().left - 20; // Limite baseado na janela
                const maxHeight = window.innerHeight - imageContent.getBoundingClientRect().top - 20;

                imageContent.style.width = `${Math.max(50, Math.min(newWidth, maxWidth))}px`; // Mínimo 50px, máximo dinâmico
                imageContent.style.height = `${Math.max(50, Math.min(newHeight, maxHeight))}px`;
                imageContent.style.flexBasis = `${Math.max(50, Math.min(newWidth, maxWidth))}px`; // Ajusta a grade
            }
        });
